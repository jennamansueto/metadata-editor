...
class Resumable_upload {
	...
	public function list_uploads()
	{
		...
		foreach ($dirs as $dir) {
			...
			try {
				$dir = $this->validate_upload_id($dir);
			} catch (Exception $e) {
				continue;
			}
	...
	/**
	 * Validate that an upload_id is a strict UUID v4 string.
	 *
	 * Upload IDs are generated by generate_upload_id() and used to build
	 * filesystem paths. Anything that does not match the exact UUID v4 shape
	 * is rejected to prevent path traversal via user-controlled identifiers.
	 *
	 * @param string $upload_id
	 * @throws Exception when the id is missing or malformed
	 * @return string The validated upload_id (lowercased)
	 */
	private function validate_upload_id($upload_id)
	{
		if (!is_string($upload_id) || $upload_id === '') {
			throw new Exception("INVALID_UPLOAD_ID");
		}

		$upload_id = strtolower($upload_id);
		$uuid_v4 = '/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/';
		if (!preg_match($uuid_v4, $upload_id)) {
			throw new Exception("INVALID_UPLOAD_ID");
		}
