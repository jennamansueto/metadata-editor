...
class Resumable_upload {
	 ...
	 *
	 * upload_ids are generated by generate_upload_id() as UUID v4 strings.
	 * Any value that does not match the UUID format is rejected so that
	 * user-controlled input can never escape $this->temp_path when it is
	 * concatenated into filesystem paths (SonarQube rule php:S2083).
	 *
	 * @param string $upload_id
	 * @return bool True if the id is a well-formed UUID
	 */
	private function is_valid_upload_id($upload_id)
	{
		return is_string($upload_id)
			&& preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i', $upload_id) === 1;
	}

	/**
	 * Require a valid upload_id or throw.
	 *
	 * @param string $upload_id
	 * @throws Exception when the id is malformed
	 */
	private function require_valid_upload_id($upload_id)
	{
	...
	{
		$this->require_valid_upload_id($upload_id);
		$metadata_path = $this->get_metadata_path($upload_id);
