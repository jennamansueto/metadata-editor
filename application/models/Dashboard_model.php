...
class Dashboard_model extends CI_Model {
    ...
    /**
     * Get storage usage statistics using OS-specific commands
     */
    public function get_storage_stats()
    {
        ...
        // If no cache, calculate using OS-specific commands
        $stats = $this->calculate_storage_stats($storage_path);
    ...
    /**
     * Calculate storage statistics using OS-specific commands
     * Returns N/A if OS commands are unavailable
     */
    private function calculate_storage_stats($storage_path)
    {
        // Use OS-specific command to get directory size
        $total_size = $this->get_directory_size_fast($storage_path);
        
        // If OS command failed, return N/A structure
        if ($total_size === null) {
            return $this->format_storage_stats_unavailable($storage_path);
        }
        
        // Get file count (optional, may not be available on all systems)
        $file_count = $this->count_files_in_directory($storage_path);
        
        return $this->format_storage_stats($total_size, $file_count, $storage_path, 'os_command');
    }

    /**
     * Format storage statistics with project type breakdown
     */
    private function format_storage_stats($total_size, $file_count, $storage_path, $calculation_method = 'os_command')
    {
        // Get storage by project type
        $this->db->select('type, COUNT(*) as count');
        $this->db->from('editor_projects');
        $this->db->group_by('type');
        $projects_by_type = $this->db->get()->result_array();

        // Calculate storage per type (based on total projects)
        $total_projects = array_sum(array_column($projects_by_type, 'count'));
        $avg_size_per_project = $total_projects > 0 ? $total_size / $total_projects : 0;
        $storage_by_type = [];
        foreach ($projects_by_type as $type) {
            $storage_by_type[] = [
                'type' => $type['type'] ?: 'unknown',
        ...
        return [
            ...
            'calculation_source' => 'exact',
            'available' => true
        ];
    }

    /**
     * Format storage statistics when calculation is unavailable
     */
    private function format_storage_stats_unavailable($storage_path)
    {
        return [
            'total_size' => null,
            ...
            'available' => false,
            'error' => 'Storage size calculation unavailable'
        ];
    }
    ...
    {
        $this->load->driver('cache');
        $this->cache->save('dashboard_storage_stats', $stats, 900); // Cache for 15 minutes
    }
    ...
    /**
     * Get directory size using OS-specific utilities
     * Returns null if OS commands are unavailable
     */
    private function get_directory_size_fast($path)
    {
        ...
        // Detect OS type using PHP_OS_FAMILY for consistency
        if (PHP_OS_FAMILY === 'Windows') {
            // Windows - use PowerShell
            return $this->get_directory_size_windows($escaped_path);
        } elseif (PHP_OS_FAMILY === 'Linux') {
            // Linux - use du -sb (bytes)
            return $this->get_directory_size_linux($escaped_path);
        } elseif (PHP_OS_FAMILY === 'Darwin') {
            // macOS - try du -sb first, then fallback to du -sk
            return $this->get_directory_size_macos($escaped_path);
        } else {
            // Other Unix-like systems - try du -sb, then du -sk
            return $this->get_directory_size_unix($escaped_path);
        }
    }

    /**
     * Get directory size on Windows using PowerShell
     */
    private function get_directory_size_windows($escaped_path)
    {
    ...
    /**
     * Get directory size on Linux using du -sb
     */
    private function get_directory_size_linux($escaped_path)
    {
        $command = 'du -sb ' . $escaped_path . ' 2>/dev/null';
        $output = @shell_exec($command);
        if (!$output) {
            return null;
        }
    ...
    /**
     * Get directory size on macOS - try du -sb first, then fallback to du -sk
     */
    private function get_directory_size_macos($escaped_path)
    {
        // Try du -sb first (bytes, preferred)
        $command = 'du -sb ' . $escaped_path . ' 2>/dev/null';
        $output = @shell_exec($command);
        if ($output) {
            $parts = preg_split('/\s+/', trim($output));
        ...
        // Fallback to du -sk (kilobytes)
        $command = 'du -sk ' . $escaped_path . ' 2>/dev/null';
        $output = @shell_exec($command);
        if ($output) {
            $parts = preg_split('/\s+/', trim($output));
            if (isset($parts[0]) && is_numeric($parts[0])) {
    ...
    /**
     * Get directory size on other Unix-like systems
     */
    private function get_directory_size_unix($escaped_path)
    {
        // Try du -sb first (bytes, preferred)
        $command = 'du -sb ' . $escaped_path . ' 2>/dev/null';
        $output = @shell_exec($command);
    ...
    public function get_activity_dashboard_data()
    {
        return [
            ...
            'total_contributors' => $this->get_total_contributors_count(),
    ...
    /**
     * Get total count of distinct contributors
     */
    public function get_total_contributors_count()
    {
        $this->db->select('COUNT(DISTINCT created_by) as count');
        $this->db->from('editor_projects');
        $this->db->where('created_by IS NOT NULL');
        $row = $this->db->get()->row_array();
    ...
    public function get_collection_stats()
    {
        ...
        // Collections with projects (using editor_collection_projects table)
        ...
        // Total project-collection assignments
        $total_assignments = $this->db->count_all('editor_collection_projects');

        return [
            'total_collections' => (int)$total_collections,
            'collections_with_projects' => $with_projects,
